...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { NextbillionSdkInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw NextbillionSdkInvalidDataException("`$name` is not set")
            is JsonNull -> throw NextbillionSdkInvalidDataException("`$name` is null")
            else -> throw NextbillionSdkInvalidDataException("`$name` is invalid, received $this")
        }
